...
const API_KEY = "AIzaSyAJ7DA5cjIiZ6TeyQo-2lNTXuEBtXkaxg4";

const toggleBtn = document.getElementById("toggle-btn");
const suggestionLinks = Array.from(
  document.getElementById("suggestionLinks").children
);
...
async function fetchVideos(searchValue) {
  ...
  try {
    const response = await fetch(
      `${BASE_URL}/search?key=${API_KEY}&part=snippet&q=${searchValue}&type=video&maxResults=40`
    );
...
function fetchChannelLogo(videoItem) {
  return fetch(
    `${BASE_URL}/channels?key=${API_KEY}&part=snippet&part=statistics&id=${videoItem.snippet.channelId}`
  )
    .then((res) => res.json())
    .then((data) => {
      videoItem.channelLogo = data.items[0].snippet.thumbnails.default.url;
      videoItem.subscriberCount = calCount(
...
async function fetchVideoStats(videoItem) {
  try {
    const response = await fetch(
      `${BASE_URL}/videos?key=${API_KEY}&part=statistics,snippet&id=${videoItem.id.videoId}`
    );
    const data = await response.json();
